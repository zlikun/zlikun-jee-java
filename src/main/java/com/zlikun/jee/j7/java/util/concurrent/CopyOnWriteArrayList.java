package com.zlikun.jee.j7.java.util.concurrent;
...
public class CopyOnWriteArrayList<E>
     ...
     * 添加元素过程中加锁，且先复制一份数据再添加元素，这样的缺点时，每次添加元素都要复制一份原数组，内存开销比较大，但好处是读取不受任何影响，即使并发环境下，也不需要加锁
     */
    public boolean add(E e) {
        // 使用ReentrantLock来保证并发条件下写入安全问题
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
            Object[] elements = getArray();
     ...
     *
     * @throws IndexOutOfBoundsException {@inheritDoc}
     * 与#add()方法相同，采用加锁和复制数组方式变更数组(这里删除元素)
     */
    public E remove(int index) {
        ...
        try {
            ...
            if (numMoved == 0)
                // 如果删除元素是最后一个元素，直接复制数组并去掉最后一个元素即可
                setArray(Arrays.copyOf(elements, len - 1));
            else {
                // 如果索引位于中间位置，刚据此分两段复制数组
